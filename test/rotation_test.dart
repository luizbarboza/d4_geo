...
void main() {
  test("a rotation of [+90°, 0°] only rotates longitude", () {
    final rotation = GeoRotation([90, 0]).call([0, 0]);
  ...
  test("a rotation of [-45°, 45°] inverse rotation of longitude and latitude",
      () {
    final rotation = GeoRotation([-45, 45]).invert([-54.73561, 30]);
  ...
  test("the identity rotation constrains longitudes to [-180°, 180°]", () {
    ...
    expect(rotate.call([180, 0])[0], equals(180));
    expect(rotate.call([-180, 0])[0], equals(-180));
    expect(rotate.call([360, 0])[0], equals(0));
    expect(rotate.call([2562, 0])[0], closeTo(42, 1e-10));
    expect(rotate.call([-2562, 0])[0], closeTo(-42, 1e-10));
  });
}
